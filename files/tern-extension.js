(function() {
  ...
  var defaultServer = null, defs = [], Pos = CodeMirror3.Pos;
  ...
  CodeMirror3.tern = {};
  CodeMirror3.tern.addDef = function(def) {
  ...
  CodeMirror3.ternHint = function(cm, c) {
    function c1(data) {
      ...
      if (CodeMirror3.templatesHint) {
        ...
        if (processTemplates) {
          CodeMirror3.templatesHint.getCompletions(cm, completions, token);
        }
      }
      c(data);
    }
  ...
  CodeMirror3.tern.showType = function(cm) {
    var server = getServer(cm);
    server.showType(cm);
  }

  CodeMirror3.tern.jumpToDef = function(cm) {
    var server = getServer(cm);
    server.jumpToDef(cm);
  }

  CodeMirror3.tern.jumpBack = function(cm) {
    var server = getServer(cm);
    server.jumpBack(cm);
  }

  CodeMirror3.tern.rename = function(cm) {
    var server = getServer(cm);
    server.rename(cm);
  }
  ...
  function getServer(cm) {
    if (defaultServer == null) {
      defaultServer = new CodeMirror3.TernServer({
        defs : defs,
  ...
  CodeMirror3.tern.getServer = getServer;
  ...
  CodeMirror3.defineOption("ternWith", false, function(cm, val, old) {
